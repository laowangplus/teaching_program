@extends('admin.public.index')
	...
	@section('content')
        ...
        <div class="page-container">
            ...
            {{--</div>--}}
            <div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l"><a href="javascript:;" onclick="removeIframe()" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 关闭选项卡</a> <a class="btn btn-primary radius" onclick="book_add('添加课程大纲','{{ url('admin/outline_add') }}')" href="javascript:;"><i class="Hui-iconfont">&#xe600;</i> 添加课程大纲</a></span> </div>
            <div class="mt-20">
                <table class="table table-border table-bordered table-bg table-hover table-sort table-responsive">
                    ...
                    <tr class="text-c">
                        ...
                        <td>{{ $course->write_user_name }}</td>
                        @if($course->type == 0)
                            <td class="amount">未通过</td>
                        @elseif(($course->type > 0))
                            <td class="amount">已通过</td>
                        @endif
                        ...
                        <td class="f-14 td-manage">
                            ...
                            <a style="text-decoration:none" class="ml-5" onClick="borrow('增减','{{ url('/admin/book_edit/'.$course->id) }}','4','','510')" href="javascript:;" title="增减"><i class="Hui-iconfont">&#xe6df;</i></a>
                            <a style="text-decoration:none" class="ml-5" onClick="publish(this,'{{ $course->id }}')" href="javascript:;" title="发布"><i class="Hui-iconfont">&#xe603;</i></a>
                            ...
                            <a style="text-decoration:none" class="ml-5" onClick="book_del(this,'{{ $course->id }}')" href="javascript:;" title="清空"><i class="Hui-iconfont">&#xe6e2;</i></a>
                        </td>
                    </tr>
                    @endforeach
                    </tbody>
    ...
    @section('script')
        ...
        <script type="text/javascript">
                ...
                layer.confirm('确认要清空该书籍吗？',function(index){
                    $.ajax({
                        ...
                        success: function(data){
                            $(obj).parents("tr").find(".amount").html("已通过");
            ...
            function publish(obj,id){
                layer.confirm('确认要发布大纲？',function(index){
                    $.ajax({
                        type: 'get',
                        url: '{{ url('admin/outline_publish') }}'+'/'+id,
                        dataType: 'json',
                        success: function(data){
